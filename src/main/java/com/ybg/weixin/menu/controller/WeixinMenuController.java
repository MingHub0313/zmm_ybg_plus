package com.ybg.weixin.menu.controller;
...
import com.ybg.weixin.menu.service.WeixinMenuService;
...
public class WeixinMenuController {
	...
	public Json create(@RequestBody WeixinButtonDTO weixinmenu) throws Exception {
		Json j = new Json();
		...
		if (weixinmenu.getIfsub() == WeixinButtonConstant.IFSUBYES) {
			...
			WeixinButtonDTO parent = weixinMenuService.get(weixinmenu.getParentid());
			if (parent == null) {
			...
			if (QvoConditionUtil.checkString(weixinmenu.getParentid())) {
				List<WeixinButtonDTO> sublist = weixinMenuService.buttonlist(weixinmenu.getParentid());
				...
				List<WeixinButtonDTO> menulist = weixinMenuService.menulist();
			...
			try {
				WeixinButtonDTO menuben = new WeixinButtonDTO();
				// 主要去掉一些属性
	...
	public List<WeixinButtonDTO> list() {
		List<WeixinButtonDTO> list = weixinMenuService.list();
	...
	public Json update(@RequestBody WeixinButtonDTO weixinmenu) throws Exception {
		Json j = new Json();
		if (!QvoConditionUtil.checkString(weixinmenu.getName())) {
			j.setSuccess(false);
			j.setMsg("名称不能为空");
	...
	private void updatebean(WeixinButtonDTO weixinmenu) {
