...
import com.ybg.base.util.SystemConfig;
 ...
 * @date 2017/1/1
 * 
 *       在线API 接口。http://localhost:8080/swagger-ui.html
 **/
@Configuration
@EnableSwagger2
public class Swagger2Configuration {
    ...
    private ApiInfo apiInfo(){
        ...
        Contact info = new Contact(config.getAuth(), config.getDomain(), config.getEmail());
        return new ApiInfoBuilder().title("云系统 APIs").description(comments).termsOfServiceUrl(SystemConstant.getSystemdomain()).contact(info).version("1.0").build();
    }
}