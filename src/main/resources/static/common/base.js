...
function DivLoadingAjaxWithFunc_Inteter(divid, url, parm, ohterfunctionname,
		iscan) {
	createloadingpic(divid);
	if (!iscan) {
		$.ajax({
...
/** 加载json 通常用在button 上* */
function JsonAjax_Inter(url, parm, dataType, button_id, ohterfunctionname,
		buttontext) {
	var loadtext = '正在执行';
	if (buttontext) {
		loadtext = buttontext;
 ...
 *            等待中的文字 ,不填 则使用正在执行
 ******************************************************************************/
function ajaxform(formid, url, parm, dataType, button_id, ohterfunctionname,
		buttontext) {
	var loadtext = '正在执行';
	if (buttontext) {
		loadtext = buttontext;
...
function createloadingpic(div) {
	// $("#" + div).html('<center><img src="/newimages3/system/loading.gif"
	// width="50px" height="50px" />加载中...</center>');
	wait();
}
/** 关闭等待中图片 * */
function closewait() {
