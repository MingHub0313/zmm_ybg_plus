...
layui
		.use(
				[ 'elements', 'jquery', 'layer', 'larry', 'navtab', 'form',
						'common' ],
				function() {
					var $ = layui.jquery, layer = layui.layer, device = layui
							.device(), elements = layui.elements(), larry = layui
							.larry(), form = layui.form(), common = layui.common;
					navtab = layui.navtab({
						elem : '#larry-tab'
					});
							...
							.ready(
									function() {
										...
										if (device.ie && device.ie < 9) {
											layer.alert('最低支持ie9，您当前使用的是古老的 IE'
													+ device.ie + '！');
										}
										// 001界面初始化
										AdminInit();
										 ...
										 * $.getJSON('../backstage/datas/top_menu.json?t=' +
										 * new Date(), { Param: 'index_menu' },
										 * function (result) { larry.set({ elem:
										 * '#menu', data: result, cached: false
										 * }); larry.render(); });
										 */
																						...
																						},
																						{
																							...
																							"children" : [
																									...
																									
																									{
																										"title" : "生成数据源管理",
																										"icon" : "larry-zhandianguanli",
																										"href" : "/gen/gendb_do/index.do"
																									},
																									
																									{
																										"title" : "生成框架主题管理",
																										"icon" : "larry-zhandianguanli",
																										"href" : "/gen/genstyle_do/index.do"
																									},
																						...
																						,
																						{
																							...
																							"children" : [
																									...
																									},
																									{
																										"title" : "表格管理",
																										"icon" : "larry-shujucaiji",
																										"href" : "/report/rtable_do/index.do"
																									}
					...
					// 常用操作
					$('#buttonRCtrl').find('dd').each(
							function() {
								$(this).on(
										'click',
										function() {
											var eName = $(this).children('a')
													.attr('data-eName');
						...
						var fScreenIndex = layer
								.alert(
										...
										function() {
											...
											$('#FullScreen')
													.html(
															'<i class="larry-icon larry-quanping"></i>退出全屏');
					...
					$('#larryTheme')
							.on(
									'click',
									function() {
										var fScreen = localStorage
												.getItem('fullscreen_info');
										var themeName = localStorage
												.getItem('themeName');
										layer
												.open({
													type : 1,
										...
										// 全屏开启
										form.on('switch(fullscreen)', function(
												data) {
											var fValue = data.elem.checked;
											localStorage.setItem(
													'fullscreen_info', fValue); // fullscreen_info:fValue

										});
										// 主题设置
										form.on('select(larryTheme)', function(
												data) {
											var themeValue = data.value;
											localStorage.setItem('themeName',
													themeValue);// themeName:themeValue
											if (themeName) {
												$("body").attr("class", "");
												$("body").addClass(
														"larryTheme-"
							...
							.bind(
									...
									function() {
										...
										if (fullscreenElement == null) {
											...
											$(this)
													.html(
															'<i class="larry-icon larry-quanping"></i>全屏');
										}
									});
					...
					function lockSystem() {
						/*
						 * var url = '../backstage/datas/lock.json'; $.get( url,
						 * function (data) { if (data == '1') {
						 * checkLockStatus(1); } else {
